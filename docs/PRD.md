...
│  ├─ JD分析：识别核心要求                                     │
│  └─ 匹配策略：制定优化方案                                   │
│  模型: GLM-4.6 | Token: ~5000 | Temperature: 0.3            │
├─────────────────────────────────────────────────────────────┤
│  阶段2: 生成+自验证（1次调用）                               │
│  ├─ 内容生成：按策略优化简历                                 │
│  └─ 自验证：标注每条内容来源                                 │
│  模型: GLM-4.6 | Token: ~6000 | Temperature: 0.5            │
├─────────────────────────────────────────────────────────────┤
│  本地验证层（代码验证，无AI调用）                             │
│  ├─ 文本相似度匹配（阈值: 0.6）                              │
│  ├─ 关键词覆盖率检查                                        │
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v1.2.0 | 2026-03-19 | 新增匹配度分数计算器 (MatchScorer)，基于明确权重规则计算分数，分数透明可解释 |
